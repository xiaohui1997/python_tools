...
def update_ssh():
    ...
    subpro("sed -i '/Port/d' /etc/ssh/sshd_config && sed -i '$aPort '" + str(
        port) + " /etc/ssh/sshd_config && iptables -C INPUT -p tcp --dport " + str(
        port) + " -j ACCEPT || iptables -I INPUT -p tcp --dport " + str(port) + " -j ACCEPT &&  systemctl restart sshd",
           "更改ssh端口为" + str(port) + "失败")
...
def zabbix():
    subpro("wget --no-check-certificate https://nchc.dl.sourceforge.net/project/zabbix/ZABBIX%20Latest%20Stable/4.2.5/zabbix-4.2.5.tar.gz", "安装zabbix软件失败")
    subpro("tar -xf zabbix-4.2.5.tar.gz", "解压失败，请稍后重试")
    subpro("useradd -s /sbin/nologin  zabbix", "创建用户zabbix失败了")
    os.chdir("zabbix-4.2.5")
...
def install_mysql():
    print('\033[1;37;32m%s\033[0m' % ("请等待系统检测是否安装mysql"))
    subpro("[ ! `which mysqld` ]", "mysql已经安装请慎重！！！")
    subpro("wget https://downloads.mysql.com/archives/get/file/mysql-5.7.26-1.el7.x86_64.rpm-bundle.tar", "mysql数据库安装失败")
    subpro("tar -xf mysql-5.7.26-1.el7.x86_64.rpm-bundle.tar ", "解压mysql数据库失败")
    subpro("rm -rf /etc/my.cnf && rm -rf /var/lib/mysql && rm -rf /usr/my.cnf", "环境比较干净")
    subpro("yum -y remove mariadb*   &&   yum -y install autoconf perl-JSON", "mysql数据库的依赖包下载失败，系统没有这个包")
    subpro("rpm -ivh *.rpm", "安装失败")
    print('\033[1;37;32m%s\033[0m' % ("mysql数据库安装成功,请去上面看初始密码,上面有提示"))
...
def install_bt():
    ...
    os.chdir("tmp_bag")
    subpro(
        "yum install -y wget && wget -O install.sh http://download.bt.cn/install/install.sh && sed -ic 's/^.*Do you want to install Bt-Panel to the.*/go='y'/' install.sh && bash install.sh",
        "下载宝塔面板失败")
    subpro("rm -rf tmp_bag", "删除软件目录失败")
    subpro("yum remove firewalld", "卸载firewalld失败")
...
def nginx_code():
    ...
    subpro(
        "yum install gcc gcc-c++ libpcre3 libpcre3-dev openssl libssl-dev perl libperl-dev pcre-devel zlib zlib-devel -y ",
        "安装nginx依赖失败")
...
def python3_install():
    ...
    # -------------------------------------------------------------------------
    os.chdir("../")
    subpro("wget https://pypi.python.org/packages/source/p/pip/pip-8.0.2.tar.gz#md5=3a73c4188f8dbad6a1e6f6d44d117eeb",
           "下载pip失败")
